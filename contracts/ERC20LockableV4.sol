...
contract ERC20LockableV4 is IERC20Lockable, Initializable, ERC20Upgradeable {
    ...
    function _beforeTokenTransfer(
        ...
        if (amount > _availableBalanceOfWhileCleaning(from))
    ...
    function _lock(
        ...
        _locks[locker][account].push(
            LockedAmount(uint224(amount), uint32(lockedUntil))
        );
    ...
    function unlock(
        ...
        (bool found, uint256 i) = _lockIndex(
            account,
            _msgSender(),
            lockedUntil
        );
    ...
    function _deleteLock(address account, address locker, uint256 i) internal {
        if (_locks[locker][account].length > 1) {
            _locks[locker][account][i] = _locks[locker][account][
                _locks[locker][account].length - 1
            ];
        ...
        for (uint256 j = 0; j < _lockers[account].length; j++) {
            if (_lockers[account][j].locker == locker) {
                if (_lockers[account][j].locks > 1)
                    _lockers[account][j].locks--;
                else {
                    _lockers[account][j] = _lockers[account][
                        _lockers[account].length - 1
                    ];
    ...
    function unlockAndTransferFrom(
        address from,
        address to,
        uint256 amount,
        uint256 tokenUntil
    ...
    function availableBalanceOf(
        ...
        for (uint256 i = 0; i < _lockers[account].length; i++) {
            for (
                uint256 j = 0;
                j < _locks[_lockers[account][i].locker][account].length;
                j++
            ) {
                if (
                    _locks[_lockers[account][i].locker][account][j]
                        .lockedUntil > block.timestamp
                ) {
                    availableBalance -= _locks[_lockers[account][i].locker][
                        account
                    ][j].amount;
                }
            }
    ...
    function _availableBalanceOfWhileCleaning(
        address account
